...
int setRTCTime() {
    ...
    // Wait for NTP result (with timeout)
    printf("waiting for ntp response\n");
    time_t ntp_time = 0;
    absolute_time_t start_time = get_absolute_time();
    
    while (ntp_time == 0 && absolute_time_diff_us(start_time, get_absolute_time()) < 60000000) {
        cyw43_arch_poll();
        sleep_ms(100);
        if (ntp_state->ntp_test_time) {
            // Get the time from the NTP response
            ntp_time = g_ntp_time;
            break;
        }
    }

    // Convert timestamp to datetime_t
    datetime_t t = {0};
    if (ntp_time != 0) {
        // Add timezone offset
